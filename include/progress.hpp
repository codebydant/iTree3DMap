...
class C_Progress {
 public:
   ...
   **/
  explicit C_Progress(unsigned long ulExpected_count = 1) { restart(ulExpected_count); }
  ...
  //  Returns: count().
  {
    if ((_count += ulIncrement) >= _next_tic_count) {
      inc_tic();
    }
    return _count;
  }
 ...
 private:
  virtual void inc_tic() { _next_tic_count = static_cast<unsigned long>((_tic / 50.0) * _expected_count); }  // inc_tic
};
