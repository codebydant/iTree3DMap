...
void Utilities::setup_argparse(int argc, char** argv) {
  arg_parser.add_description("The program estimate dendrometric features of an individual tree\nfrom a 3D real scale mapping using a images sequence\nusing openMVG and PMVS2.");

  arg_parser.add_argument("--project-name").default_value(std::string("custom_project")).help("project name description");
  arg_parser.add_argument("-i", "--input-dir").required().help("path to images folder");
  arg_parser.add_argument("--focal-length").default_value(float(30)).scan<'g', float>().help("camera focal length");
  arg_parser.add_argument("--world-reference").default_value(float(10)).scan<'g', float>().help("world reference in (cm)");
...
bool Utilities::run_openMVG() {
  std::string project_name = arg_parser.get<std::string>("--project-name");
  std::string input_dir = arg_parser.get<std::string>("--input-dir");
  std::string output_dir = arg_parser.get<std::string>("--output-dir");
  std::string project_dir = os::path::join(output_dir, project_name);
  std::string project_pcd_file = os::path::join(project_dir, "3D_Mapping", "MAP3D.pcd");
  std::string project_plyfile = os::path::join(project_dir, "3D_Mapping", "MAP3D.ply");

  itreemap::OpenMVGInterface sfm_mapper;
  sfm_mapper.compute_sfm(input_dir, project_dir, Map3D);
  sfm_mapper.export_pcd_file(project_pcd_file, Map3D);
  sfm_mapper.export_ply_file(project_plyfile, Map3D);
...
bool Utilities::getScaleFactor(double& scale_factor) {
  /*
  ...
  */
  ...
  std::string output_dir = arg_parser.get<std::string>("--output-dir");
  std::string project_dir = os::path::join(output_dir, project_name);
  // std::string project_pcd_file = sfm_mapper.get_project_pcd_file();
  // std::string world_reference = std::to_string(scale_module.get_world_reference());

  // //  if (boost::filesystem::exists(project_dir) && boost::filesystem::exists(project_pcd_file)) {
  // // std::cout << blue << "\nProject Found!" << reset << std::endl;

  itreemap::ScaleFactorModule scale_module;
  bool success = scale_module.convert_sfm_data_to_xml(project_dir);
  if (not success) {
    return false;
  }
  ...
  success = scale_module.loadSFM_XML_Data(intrinsic, cameras_poses, project_dir);
  if (not success) {
    return false;
  }

  PCL_INFO("\nImages: %lu", scale_module.images_filenames.size());
  ...
  int minRadius = 0;
  int maxRadius = 150;
  std::string houghOutputParameters = project_dir;
  ...
  while (true) {
    ...
    if (img_selected.size() <= 0) {
      ...
      std::string image_path = os::path::join(project_dir, "image_selected.txt");
      ...
      std::getline(infile, img_iid);
      infile.close();
      // std::string line;

      // if (img_path.is_open()) {
      //     while (std::getline(img_path, line)) {
  ...
  std::cout << "img_selected: " << img_selected << std::endl;
  ...
  */
  ...
  //   std::ofstream cir(dendrometric_results.c_str());

  //   std::string dendrometric_results2 = output_dir;
  //   dendrometric_results2 += "/pts_for_circle2.txt";

  //   std::ofstream cir2(dendrometric_results2.c_str());
...
bool Utilities::densifyWithPMVS(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& output_cloud4, bool& projectFound) {
  ...
  std::string cloudPLY = project_dir;
...
bool Utilities::uniformScaling(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_scaled, bool& projectFound, double& scale) {
  std::string project_name = arg_parser.get<std::string>("--project-name");
  std::string input_dir = arg_parser.get<std::string>("--input-dir");
  std::string output_dir = arg_parser.get<std::string>("--output-dir");
  std::string project_dir = os::path::join(output_dir, project_name);
  ...
  std::string prefix1 = project_dir;
  ...
  std::string prefix2 = project_dir;
  prefix2 += "/";
...
// void Utilities::create_mesh(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,pcl::PolygonMesh &mesh,int nThreads,int setKsearch, int depth, float pointWeight,float samplePNode,
// float scale, int isoDivide, bool confidence, bool outputPolygons, bool manifold, int solverDivide){
...
// void Utilities::alignCloudSFM(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud,pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_transformed,Eigen::Affine3f& transform_1,Eigen::Affine3f&
// transform_2,Eigen::Affine3f& transform_3){
