...
bool Segmentation::extractTree(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, const std::string& output_path, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_aligned,
                               pcl::PointCloud<pcl::PointXYZ>::Ptr& trunk_cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr& tree_segmented,
  ...
  std::cout << "PointCloud after filtering has: " << crown_cloud->points.size() << " data points." << std::endl;
...
bool Segmentation::trunkSegmentation(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_aligned,
                                     pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_without_trunk, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_trunk, bool setGUI,
                                     Eigen::Matrix4f align_cloud, int K, double distanceWeight, int maxIterations, double distanceThreshold, double distanceWeight_cylinder,
                                     int maxIterations_cylinder, double distanceThreshold_cylinder, double minRadius, double maxRadius) {
  if (cloud->points.size() <= 0) {
    PCL_ERROR("Input point cloud has no data!\n");
    return false;
  }
  ...
  if (setGUI) {
    ...
    while (file >> K >> distanceWeight >> maxIterations >> distanceThreshold >> distanceWeight_cylinder >> maxIterations_cylinder >> distanceThreshold_cylinder >> minRadius >>
           maxRadius) {
    }
  ...
  for (pcl::PointCloud<pcl::PointXYZ>::iterator it = trunk_seg_filtered->begin(); it != trunk_seg_filtered->end(); ++it, ++my_progress_bar) {
  ...
  */
  ...
  if (minH.y < 0) {
    ...
    align_cloud << 1, 0, 0, 0, 0, 1, 0, -offsetY, 0, 0, 1, 0, 0, 0, 0, 1;
  }
...
bool Segmentation::crownSegmentation(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_crown, pcl::PointCloud<pcl::PointXYZ>::Ptr& crown_better_segmented, bool setGUI,
                                     int octreeResolution, double eps, int minPtsAux, int minPts) {
...
void Segmentation::MultipleViewportsVTK(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud1, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud2) {
